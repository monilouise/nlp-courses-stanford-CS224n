...
def softmax(x):
    ...
    ### YOUR CODE HERE
    max = tf.reduce_max(x, reduction_indices=[1], keep_dims=True)
    out = tf.exp(x - max) / tf.reduce_sum(tf.exp(x - max), reduction_indices=[1], keep_dims=True)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    each = - tf.reduce_sum(tf.to_float(y) * tf.log(yhat), reduction_indices=[1], keep_dims=True)
    out = tf.reduce_sum(each)
    ### END YOUR CODE
