...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        for idw, word in enumerate(sentence):
            flat_window = []
            if idw == 0:
                flat_window += start
            start_idx = max(0, idw - window_size)
            end_idx = min(sentence_len, idw + window_size + 1)
            window = sentence[start_idx:end_idx]
            flat_window += [feature for word in window for feature in word]
            label = labels[idw]
            windowed_data.append((flat_window, label))
        flat_window += end
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(shape=
                            ...
                            initializer=xav, name="W")

        b1 = tf.get_variable(shape=self.config.hidden_size, name="b1", dtype=tf.float32, initializer=xav)
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, dropout_rate)
        U = tf.get_variable(shape=(self.config.hidden_size, self.config.n_classes),
                            initializer=xav, name="U")
        b2 = tf.get_variable("b2", shape=[self.config.n_classes], dtype=tf.float32, initializer=xav)
