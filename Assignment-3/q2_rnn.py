...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        length = len(sentence)
        mask = ([True for s in sentence] + [False] * max(0, max_length - length))[0:max_length]
        sentence = (sentence + ([zero_vector] * (max(0, max_length - length))))[0:max_length]
        labels = (labels + [zero_label] * max(0, max_length - len(labels)))[0:max_length]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable("U", shape=(Config.hidden_size, Config.n_classes), initializer=xav)
        b2 = tf.get_variable("b2", shape=Config.n_classes, initializer=xav)
        h = tf.zeros(shape=(1, Config.hidden_size))
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(unstacked_x[time_step], h)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        tf.boolean_mask(self.labels_placeholder, self.mask_placeholder)
        tf.boolean_mask(preds, self.mask_placeholder)
        loss = tf.reduce_mean(
